...
> 当前版本：**2.5.4（Build 44）**。项目面向 macOS 26 和 Apple Silicon 构建，不提供旧系统兼容层。
...
- 调整全局字体层级，并围绕克制的外壳表面、稳定遥测面板和更清晰的间距重新组织 macOS 26 Liquid Glass 主窗口与菜单栏。
- 将侧边栏改为四角完整、与窗口边缘留白的独立玻璃卡片，并统一外壳、面板、卡片、控件和微型元素的连续圆角层级。
- 压缩状态 Header 与 360 pt 菜单面板，移除嵌套玻璃和大面积白色表面，同时将应用管理从多张大卡片改为紧凑分组列表。
...
- 菜单栏弹窗加入实时进程流量 Top 3，共享主面板采样结果，不重复启动采集器。
...
- macOS 26 原生 Liquid Glass 用于侧边栏、状态 Header、进程筛选外壳和菜单根面板；密集菜单使用更稳健的 Regular 玻璃，按钮使用 `.glass` / `.glassProminent`。
- 实时遥测、历史趋势和长列表保持稳定半透明表面，避免高频刷新触发大面积玻璃重新采样。
- 全局文字使用 11–13 pt 起步的语义字号层级，时间戳与单位保留紧凑等宽样式。
- 常驻侧边栏按概览和管理工具分组，模块切换自动返回内容顶部。
- 每个模块拥有一张结果优先的状态 Header，以标题、状态、关键数值和单一主操作建立视觉焦点。
- 环境光晕与透明窗口同时支持浅色与深色外观，并保留原生窗口控制按钮。
...
- 菜单栏弹窗可重新打开主面板或退出程序。
- 菜单栏弹窗展示当前进程流量 Top 3，以及每个进程的实时下载和上传速度。
- 菜单弹窗的系统指标和进程流量 Top 3 均约每 2 秒更新，主面板进程流量同样约每 2 秒更新。
...
从 [GitHub Releases](https://github.com/svsvnm/MacResourceMonitor/releases/latest) 下载 `MacResourceMonitor-2.5.4.zip`，解压后将“Mac资源监控.app”拖入“应用程序”目录。

推送匹配的 `v*` 标签后，GitHub Actions 会在 macOS 26 构建机上从该标签源码生成 Release 压缩包，上传 ZIP 与 SHA-256 校验文件并自动发布 GitHub Release。应用使用 ad-hoc 签名，目前未进行 Apple Developer ID 签名或公证；如 Gatekeeper 阻止首次打开，请在 Finder 中右键应用并选择“打开”，确认来源后继续。
